...
def prognoz(request):
    if (request.method == 'POST'):
        ...
        if len(res) == 2:
            ...
            all_info = City_Info(date_get, res['list'], all_info, checkbox)

            if len(all_info) == 0:
                city = "Введена некорректная дата"

        context = {'all_info': all_info, 'city': city, 'checkbox': checkbox}
...
def City_Info(date_get, res, all_info, checkbox):
    for elem in res:
        ...
        if date_get == dt[0] or date_get == "-":
            if checkbox == "false":
                ...
                'napr': elem['wind']['deg'],
                'water': elem['main']['humidity'],
                'davl': elem['main']['pressure']
                }
